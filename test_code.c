...
//	double frequency_check;
	...
	int test_freq =10;
	/*
	* For testing, uncomment the next three lines and use any image, threshold and frequency
  ...
  float final_image[M][N];
	...
	*/
	...
	//for(a=1; a<=1501; a++) { //For comparison against serial version, use instead of while loop
		local_max  = 0;
	...
	/*
	* final_local_array has to be static and of size 
	* i_length x j_length for MPI_Send => can't dynamically allocate
	*/
	...
	*/
	...
	/*
	* Receive all images and place in master, then print output
	*/
	if(rank==0) { 
		...
		*/  
		for(a=0; a<size-1; a++) {
			...
			/*
			*	 Work out out how big the incoming message will be based on the 
			*	 same routine as above, depending on which rank has issued the 
			*	 most recent send
			*/
		...
		*/
		...
		pgmwrite(output_image, final_image, M, N);
		 ...
		 printf("%d \t %lf \t %lf \t %lf \t %lf \t %d \t %s \t %d \t %f \n", size, final_pixel_average, max_change, time_taken, time_in_loop, final_iter_count, image_to_read, test_freq, threshold); //Used for testing
	} // end if(rank==0)
